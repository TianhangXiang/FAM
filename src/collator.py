...
class TrainCollator:
    ...
    def __call__(self, examples):
        ...
        qry_inputs = process_vlm_inputs_fns[self.model_args.model_backbone](qry_inputs,
                                        processor = self.processor,
                                        max_length = self.data_args.max_len)
        pos_inputs = process_vlm_inputs_fns[self.model_args.model_backbone](pos_inputs,
                                        processor = self.processor,
                                        max_length = self.data_args.max_len)

        qry_pure_vision = []
        tgt_pure_vision = []
        dataset_name = [e['dataset'] for e in examples]
        
        for example in examples:
            qry_pure_vision_indicator = example['qry_pure_vision_indicator']
            qry_pure_vision.append(qry_pure_vision_indicator)
            tgt_pure_vision_indicator = example['tgt_pure_vision_indicator']
            tgt_pure_vision.append(tgt_pure_vision_indicator)
            
        # update the pure vision representation indicator here
        qry_inputs.update({'pure_vision': qry_pure_vision,
                                        'dataset_name': dataset_name},
                                       )
        pos_inputs.update({'pure_vision': tgt_pure_vision,
                                        'dataset_name': dataset_name},
                                       )
...
class EvalCollator:
    ...
    def __call__(self, examples):
        ...
        """
        if len(examples[0]) == 3:
            examples = {'text': [e[0] for e in examples], 'image': [e[1] for e in examples], 'label': [e[2] for e in examples]}
        else:
            examples = {'text': [e[0] for e in examples], 'image': [e[1] for e in examples]}
        # examples = {'text': [e[0] for e in examples], 'image': [e[1] for e in examples]}
